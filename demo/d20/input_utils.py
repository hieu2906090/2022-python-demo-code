def fntds(cauThongBao, danhSachKey):
    print(cauThongBao)
    ds = input().split(',')
    ds = list(map(lambda x: int(x), ds))
    return ds


def traThongTinTheoDS(cauThongBao, danhSachKey):
    print(cauThongBao)
    return input().split(',')
...
def fn(cauThongBao, danhSachKey, nhapThongTinFn):
    tiepTucHayKhong = True
    thongTinList = []
    while tiepTucHayKhong:
        ttHsDict = nhapThongTinFn(cauThongBao, danhSachKey)
